...
export class UsuarioServicio {
  ...
  postUsuario(usuario:Usuario):Observable<Usuario>{
    ...
    return this.http.post<Usuario>(`${this.API_URL}/users.json`, usuario);
  }

  //metodo buscar por id
  //observable<: se usa cuando tenemos que establecer coneccion con una fuente de datos externa
  getUsuarioById(id:string): Observable<Usuario>{
    return this.http.get<Usuario>(`${this.API_URL}/users/${id}.json`)
  }


  //metodo put - tiene dos parametros
  putUsuario(id:string, usuario:Usuario): Observable<Usuario>{
    return this.http.put<Usuario>(`${this.API_URL}/users/${id}.json`, usuario)
  }

  //metodo delete
  deleteUsuario(id: string): Observable<void>{
    return this.http.delete<void>(`${this.API_URL}/users/${id}.json`);
  }
}
